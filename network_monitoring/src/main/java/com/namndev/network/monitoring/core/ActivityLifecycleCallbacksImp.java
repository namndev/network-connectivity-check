...
public final class ActivityLifecycleCallbacksImp implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityCreated(@NonNull Activity activity, @Nullable Bundle p1) {
        if( !(activity instanceof LifecycleOwner)) return;
        if (activity instanceof FragmentActivity)
            addLifecycleCallbackToFragments((FragmentActivity) activity);

        if ( !(activity instanceof NetworkConnectivityListener)
                || !((NetworkConnectivityListener) activity).getShouldBeCalled()) return;
        ...
        NUtils.safeRun("ActivityCallbacks", () -> {
            FragmentManager.FragmentLifecycleCallbacks callbacks =  new  FragmentManager.FragmentLifecycleCallbacks() {
                ...
                public void onFragmentCreated(@NonNull FragmentManager fm, @NonNull Fragment f, @Nullable Bundle savedState) {
                    if (!(f instanceof NetworkConnectivityListener) || !((NetworkConnectivityListener) f).getShouldBeCalled()) return;
